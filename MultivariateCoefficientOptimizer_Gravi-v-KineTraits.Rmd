...
newTipAngleData = predictor(as.matrix(tipAngle), optBB)

# estimate the % var explained by solution vector
var = var(tipAngle[,1]) + var(tipAngle[,2]) + var(tipAngle[,3])

var2 = var(newTipAngleData)
var2/var

# Conclusion: 20% of variance is explained by the newTipAngleData

# St Dev
sqrt(var2) # st dev of newTipAngleData
...
tipAngle = read_csv("~/Desktop/GravitropismAssay/TipAngle-Parameters-KinematicTraits_Data/2021-11-30_tipAngle_PCScores.csv")
tipAngle = t(tipAngle) # Makes this 3 x 162
tipAngle = tipAngle[,1]
...
tipAngles_Ex = tipAngles_Ex[2:242,]

plot(uta) # plot of averaged tip angle over time

G = predictor(eta, as.numeric(as.vector(tipAngle))) + uta
plot(G, ylim=c(0, 80))
plot(uta)
plot(tipAngles_Ex[,1])
