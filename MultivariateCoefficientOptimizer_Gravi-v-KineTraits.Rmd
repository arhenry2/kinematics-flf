...
X2 = read.csv("~/Desktop/GravitropismAssay/TipAngle-Parameters-KinematicTraits_Data/RIL1_GravitropismData_tipAngles.csv") # dataset 2
...
S1_PCScores_all = X1_scores$x
S1_PCScores_allwRILs = data.frame(cbind(X1$RILs, S1_PCScores_all))
S1_mean = S1_PCScores_allwRILs %>%
  group_by(V1) %>%
  summarise_each(funs(mean(., na.rm = TRUE)))
# need to add rows 89 & 5 b/c no data for them (same for 115?)
write.csv(S1_mean, "~/Desktop/GravitropismAssay/TipAngle-Parameters-KinematicTraits_Data/2022-01-26_REGR_PCScores.csv") # add in rows with NAs
S1 = read.csv("~/Desktop/GravitropismAssay/TipAngle-Parameters-KinematicTraits_Data/2022-01-26_REGR_PCScores.csv")
S1 = S1[,2:5]
...
T3 <- function(s){
  ...
  Pc = predictor_norm(as.matrix(S1), as.vector(params))
  Sc = predictor_norm(as.matrix(S2), as.vector(scrs))
...
# REGR data, Tip Angle 1, & TipAngle 2 and put them on the network drive
## including PC scores
```
