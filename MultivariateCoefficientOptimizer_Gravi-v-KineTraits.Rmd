...
``` {r actually-putting-it-together}
# T3 combines the 2 datasets below to see which "recipe" gives the best correlation between these:
## Raw REGR & tipAngle data
## LOD scores of mapped, raw REGR & tipAngle data
T3 <- function(s){
  
  alpha = s[1:4]
  beta = s[5:6] # specify this as "end of s" later
  
  H = metric(REGR, tipAngle)
  H = as.vector(H)
...
recipe = fminsearch(T3, runif(6), minimize = FALSE, maxiter = 100)
pp = recipe$xmin
optJ = pp[1:4]
optK  = pp[5:6]
