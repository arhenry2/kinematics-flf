...
## Note: The following code works, but replace the original data in the table
## How to mutate rows so I can have means, sd, var, & n in same table?

# Aggregation to find mean
AllRILs_SummaryParameters %>% 
  group_by(RILs) %>% #grouped by RIL variable
  summarise_each(funs(mean = mean(., na.rm = TRUE))) 

# Variance of each RIL's parameters
AllRILs_SummaryParameters %>% 
  group_by(RILs)%>%
  summarise_each(funs(var = var(., na.rm = TRUE)))

# St. dev of each RIL's parameters
AllRILs_SummaryParameters %>% 
  group_by(RILs)%>%
  summarise_each(funs(sd = sd(., na.rm = TRUE)))
